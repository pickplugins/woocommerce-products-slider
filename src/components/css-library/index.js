...
function Html(props) {
	...
	function fetchCss() {
		...
		fetch("https://comboblocks.com/server/wp-json/wcps/v2/get_post_css", {
			method: "POST",
			headers: {
			...
			<PGtabs
				...
				<PGtab name="cssItems">
											...
											<div className="absolute top-0 right-2">
												{!x.is_pro && (
													<span className=" bg-lime-600 px-2 py-1  no-underline rounded-sm  cursor-pointer text-white">
														{__("Free", "woocommerce-products-slider")}
													</span>
												)}
												{x.is_pro && (
										...
										{x.is_pro && isProFeature && (
											<div className="">
												<a
													...
													<span>{__("Subscribe to Import", "woocommerce-products-slider")}</span>
												</a>
											</div>
										)}
													...
													<Icon
														icon={
															<svg
																...
																viewBox="0 -6 16 16"
																version="1.1">
																<title>{__("Apply Style", "woocommerce-products-slider")}</title>
																<defs>
																	<linearGradient
																		id="gradient1"
																		x1="7.91394"
													...
													<Popover position="bottom left">
														...
														>
															<div
																...
																{__("Apply All", "woocommerce-products-slider")}
															</div>
					...
					<div
						...
						{__("Load More", "woocommerce-products-slider")}
					</div>
				</PGtab>
				<PGtab name="submit">
					<div>
						<label htmlFor="">{__("Item Title", "woocommerce-products-slider")}</label>
						<InputControl
							className="w-full"
					...
					<PanelRow>
						<label htmlFor="">{__("Choose category", "woocommerce-products-slider")}</label>
						<SelectControl
							className="w-full"
							style={{ margin: 0 }}
							label=""
					...
					<div>
						<label htmlFor="">{__("Add Some Tags", "woocommerce-products-slider")}</label>
						<InputControl
							className="w-full"
					...
					<div className="my-4">
						<div
							...
							{__("Take Screenshot", "woocommerce-products-slider")}
						</div>
						<label htmlFor="">{__("Preview Thumbnail", "woocommerce-products-slider")}</label>
						<img src={cssSubmission.thumb} />
					</div>
					<div>
						<label htmlFor="">{__("Your Email", "woocommerce-products-slider")}</label>
						<InputControl
							className="w-full"
					...
					<div
						...
						onClick={(ev) => {
							...
							postData = JSON.stringify(postData);
							fetch(
								"https://comboblocks.com/server/wp-json/wcps/v2/submit_css",
								{
									method: "POST",
									headers: {
										"Content-Type": "application/json;charset=utf-8",
						...
						{__("Submit to CSS Library", "woocommerce-products-slider")}
