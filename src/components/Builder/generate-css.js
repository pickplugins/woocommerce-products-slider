...
function Html(props) {
	...
	useEffect(() => {
		var styleObjX = {};
		if (wcpsData?.loopLayout && wcpsData.loopLayout.length > 0) {
			const flatData = flattenObject(wcpsData.loopLayout);
			flatData.map(item => {
		...
		var itemWrapCss = generateElementCss(wcpsData.itemWrap, itemWrapSelector);
		...
		var paginationItemCss = generateElementCss(wcpsData.paginationItem, paginationItemSelector);
		Object.entries(paginationItemCss).map((selectors) => {
			var selector = selectors[0];
			var selectorData = selectors[1];
			styleObjX[selector] = selectorData;
		});
		var paginationItemActiveCss = generateElementCss(wcpsData.paginationItemActive, paginationItemActiveSelector);
		...
		var navItemCss = generateElementCss(wcpsData.navItem, navItemSelector);
