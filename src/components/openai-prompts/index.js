...
function Html(props) {
  ...
  var promptGenerateTestimonial = [
    { label: "Choose Prompt", value: '' },
    { label: "Create 5 wcpss", value: "Create 5 wcpss where each item has the person's name, their job role, a short message praising the efficiency of the service, and their website link." },
    { label: "Write  4 wcpss", value: "Write  4 wcpss, Include the name, role, an inspiring message about how the product improved their workflow, and their website." },
      ...
      <div className=''>
        ...
        <div className="flex items-center gap-3 my-3">
          <div className='cursor-pointer text-center my-3 bg-gray-700 hover:bg-gray-600 rounded-sm text-white px-3 py-2' onClick={ev => {
            ...
            {isLoading && (
              <span> {__("Please wait...", "woocommerce-products-slider")}</span>
            )}
            {!isLoading && (
              <span> {__("Get Response", "woocommerce-products-slider")}</span>
            )}
            {isLoading && (
              <Spinner />
            )}
...

          {
            ...
            && (
              <div>
                ...
                <ToggleControl
                  label={__("Auto Update?", "woocommerce-products-slider")}
                  className="mt-3"
